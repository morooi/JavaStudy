...
# 1. 修改单表的记录
# 案例 1: 修改 beauty 表中姓唐的女神的电话为 13899888899
UPDATE beauty
SET phone = '13899888899'
WHERE NAME LIKE '唐%';

# 案例 2: 修改 boys 表中 id 为 2 的名称为张飞, 魅力值 10
UPDATE boys
SET boyName = '张飞',
    userCP  = 10
WHERE id = 2;
...
# 2. 修改多表的记录
# 案例 1: 修改张无忌的女朋友的手机号为 114
UPDATE beauty b
    JOIN boys bo ON b.boyfriend_id = bo.id
SET b.phone = 114
WHERE boyName = '张无忌';

# 案例 2: 修改没有男朋友的女神的男朋友编号都为 2 号
UPDATE beauty b
...
DELETE b
FROM beauty b
         JOIN boys bo ON boyfriend_id = bo.id
WHERE boyName = '张无忌';

# 案例: 删除黄晓明的信息以及他女朋友的信息
DELETE b, bo
FROM boys bo
         JOIN beauty b ON boyfriend_id = bo.id
WHERE boyName = '黄晓明';

# 方式二: truncate 语句
# 案例: 将魅力值 > 100 的男神信息删除 (不允许where, truncate 是清空表中所有数据)
TRUNCATE TABLE boys;
